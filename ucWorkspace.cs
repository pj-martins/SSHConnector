...
namespace SSHConnector
{
	public partial class ucWorkspace : UserControl
	{
		...
		private void frmCreateUpdate_Load(object sender, EventArgs e)
		{
			...
			txtUser.Text = Terminal.User;
			if (!string.IsNullOrEmpty(Terminal.PasswordEncrypted))
			{
				txtPassword.Text = PaJaMa.Common.EncrypterDecrypter.Decrypt(Terminal.PasswordEncrypted, Terminal.PWD);
			}
		...
		private void saveSettings()
		{
			...
			Terminal.User = txtUser.Text;
			if (string.IsNullOrEmpty(txtPassword.Text))
			{
				Terminal.PasswordEncrypted = string.Empty;
			}
			else
			{
				Terminal.PasswordEncrypted = PaJaMa.Common.EncrypterDecrypter.Encrypt(txtPassword.Text, Terminal.PWD);
			}
		...
		private void runSSH(Terminal terminal, bool captureError = false)
		{
			...
			arguments += $" -ssh {terminal.Host}";
			if (terminal.Port != 0) arguments += $" -P {terminal.Port}";
			if (!string.IsNullOrEmpty(terminal.User)) arguments += $" -l {terminal.User}";
			if (!string.IsNullOrEmpty(terminal.PasswordEncrypted))
			{
				var pwd = PaJaMa.Common.EncrypterDecrypter.Decrypt(terminal.PasswordEncrypted, Terminal.PWD);
				arguments += $" -pw {pwd}";
			}
			if (!string.IsNullOrEmpty(terminal.Key)) arguments += $" -i {terminal.Key}";
			if (!string.IsNullOrEmpty(terminal.TunnelDestination))
			{
				arguments += $" -nc {terminal.TunnelDestination}:{terminal.TunnelPort}";
			}
			// var startInf = new ProcessStartInfo("cmd.exe", arguments);
			// var startInf = new ProcessStartInfo("ssh.exe", arguments.Trim());
			var startInf = new ProcessStartInfo("putty.exe", arguments.Trim());
		...
		private void btnConnect_Click(object sender, EventArgs e)
		{
			...
			if (!File.Exists("putty.exe")) File.WriteAllBytes("putty.exe", Properties.Resources.putty);
			//new Thread(new ThreadStart(() =>
			//{
				runSSH(Terminal);
			// })).Start();
		...
		private void btnExplore_Click(object sender, EventArgs e)
		{
			...
			frm.SettingsChanged += Frm_LastDownloadedChanged;
			frm.Show();
			//var shell = clnt.CreateShell();
			//shell.
		}

		private void Frm_LastDownloadedChanged(object sender, EventArgs e)
		{
			this.TerminalSettingsUpdated.Invoke(sender, e);
		}
	}
}
